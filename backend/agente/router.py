...
# La Bóveda (bitácora de acciones de Cost, Ciclo 3) ya NO tiene endpoints
# REST propios — se consulta y se deshace SOLO desde dentro de la
# conversación, vía las tools `agente_bitacora_consultar`/
# `agente_bitacora_deshacer` (ver `agente/tools/bitacora.py`), nunca
# directamente desde el frontend. El barrido de limpieza por retención vive
# aparte, en `routers/agente_cron.py` (sin sesión de usuario, como
# `proyectos_cron.py`).
